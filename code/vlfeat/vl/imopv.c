...
/** @fn ::vl_imsmooth_smooth_sub_image(VlImSmoothFilt*,float*,vl_size,float const*,vl_size,vl_size,vl_size,double)
 ** @brief Smooth a sub-image with a gaussian filter.
 ** @param self        Smooth filter.
 ** @param outputImage output image buffer.
 ** @param outStride   width of the output image including padding.
 ** @param inputImage  input image buffer.
 ** @param inWidth     input image width.
 ** @param inHeight    input image height.
 ** @param inStride    width of the input image including padding.
 ...
 **
 ** This function allows to smooth of sub-images when the strides of input and
 ** output images are correct. The strides are usually numbers of collumns.
 **
...
vl_imsmooth_smooth_sub_image (VlImSmoothFilt *self,
                              ...
                              double sigma)
{
  size_t tmp_sz_needed = inWidth * inHeight * sizeof(float);
  ...
  if (self->gaussFilterSigma != sigma) {
    ...
    self->gaussFilterWidth = VL_MAX(ceil(3.0 * sigma), 1) ;
    self->gaussFilterSigma = sigma ;
    self->gaussFilter = vl_malloc (sizeof(float) * (2 * self->gaussFilterWidth + 1)) ;

    for (j = 0 ; j < 2 * self->gaussFilterWidth + 1 ; ++j) {
      float d = ((float)((signed)j - (signed)self->gaussFilterWidth)) / ((float)sigma) ;
      self->gaussFilter[j] = (float) exp (- 0.5 * (d*d)) ;
  ...
  if (self->gaussFilterWidth == 0) {
    if ((outStride == inWidth) && (inStride == inWidth))
      memcpy (outputImage, inputImage, sizeof(float) * inWidth * inHeight) ;
    else {
      /* If the images are not continuous, copy by rows */
      vl_uindex j;
      for(j = 0; j < inHeight; ++j) {
        memcpy (outputImage + outStride * j, inputImage + inStride * j,
                sizeof(float) * inWidth) ;
      }
    }
    return ;
  }
  ...
  if (tmp_sz_needed > self->temp_sz) {
    if (self->temp)
      vl_free(self->temp);
    self->temp = vl_malloc(tmp_sz_needed);
    self->temp_sz = tmp_sz_needed;
  }

  vl_imconvcol_vf (self->temp, inHeight,
                   inputImage, inWidth, inHeight, inStride,
                   self->gaussFilter,
                   - self->gaussFilterWidth, self->gaussFilterWidth,
                   1, VL_PAD_BY_CONTINUITY | VL_TRANSPOSE) ;

  vl_imconvcol_vf (outputImage, outStride,
                   self->temp, inHeight, inWidth, inHeight,
                   self->gaussFilter,
                   - self->gaussFilterWidth, self->gaussFilterWidth,
                   1, VL_PAD_BY_CONTINUITY | VL_TRANSPOSE) ;
}
...
/** ------------------------------------------------------------------
 ** @fn vl_imsmooth_new(int,int)
 ** @brief Create a new Gaussian Smooth filter
 **
 ** @param width    image width.
 ** @param height   image height.
 **
 ** The function allocates and returns a new Smooth filter for the
 ** specified image size.
 **
 ** @return the new Smooth filter.
 ** @sa ::vl_imsmooth_delete().
 **/
#ifndef VL_IMOPV_INSTANTIATING
VL_EXPORT VlImSmoothFilt *
vl_imsmooth_new (int width, int height)
{
  VlImSmoothFilt *f = vl_malloc (sizeof(VlImSmoothFilt)) ;

  int nel = width * height ;

  f-> width   = width ;
  f-> height  = height ;

  f-> temp_sz = sizeof(float) * nel;
  f-> temp    = vl_malloc (f->temp_sz) ;
...
/** -------------------------------------------------------------------
 ** @fn vl_imsmooth_delete(VlImSmoothFilt*)
 ** @brief Delete Smooth filter
 **
 ** @param f Smooth filter to delete.
 **
...
/** ------------------------------------------------------------------
 ** @fn vl_imgradient_polar_callback(float const*,int,int,float*,int,int,int,int,void*)
 ** @brief Callback for ::vl_scalespace_apply wrapping ::vl_imgradient_polar()
 **/
#ifndef VL_IMOPV_INSTANTIATING
VL_EXPORT void
vl_imgradient_polar_f_callback  (float const *sourceImage,
                                 ...
                                 float *dstImage,
                                 int VL_UNUSED(dstWidth), int VL_UNUSED(dstHeight),
                                 int VL_UNUSED(octave), int VL_UNUSED(level),
                                 void VL_UNUSED(*params))
{
  vl_imgradient_polar_f(dstImage, dstImage+1, 2, 2*sourceImageWidth,
                        sourceImage, sourceImageWidth,
