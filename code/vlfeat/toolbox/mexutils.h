...
/** ------------------------------------------------------------------
 ** @brief Export scale space into matlab structure
 ** @param scsp Pointer to the scale space to be xported
 ** @return Pointer to matlab structure with the scale space
 **/
static mxArray *
vlmxCreateArrayFromScaleSpace(VlScaleSpace const *scsp)
{
  mxArray *array = NULL ;
  mxArray *s_idxs = NULL;
  mxArray *o_idxs = NULL;
  mxArray *data = NULL;
  double* ptr;
  vl_index o_min, o_max, o_num, s_min, s_max, s_num;
  vl_index o_i, s_i, o, s;

  if (scsp == NULL) {
    return mxCreateDoubleMatrix(0,0,mxREAL);
  }

  o_min = vl_scalespace_get_octave_min(scsp);
  o_max = vl_scalespace_get_octave_max(scsp);
  s_min = vl_scalespace_get_level_min(scsp);
  s_max = vl_scalespace_get_level_max(scsp);

  o_num = o_max - o_min + 1;
  s_num = s_max - s_min + 1;

  {
    const char* names[] = {"o_idxs", "s_idxs", "data"};
    array = mxCreateStructMatrix(1, 1, 3, names);
  }

  o_idxs = mxCreateNumericMatrix(1, o_num, mxDOUBLE_CLASS, mxREAL);
  ptr = mxGetPr(o_idxs);
  for (o = o_min; o <= o_max; ++o) {
    *ptr++ = (double)o;
  }
  mxSetFieldByNumber(array, 0, 0, o_idxs);

  s_idxs = mxCreateNumericMatrix(1, s_num, mxDOUBLE_CLASS, mxREAL);
  ptr = mxGetPr(s_idxs);
  for (s = s_min; s <= s_max; ++s) {
    *ptr++ = (double)s;
  }
  mxSetFieldByNumber(array, 0, 1, s_idxs);

  data = mxCreateCellMatrix(o_num, s_num);

  o_i = 0;
  for (o = o_min; o <= o_max; ++o) {
    int w = (int)vl_scalespace_get_octave_width(scsp, o);
    int h = (int)vl_scalespace_get_octave_height(scsp, o);
    vl_size num_el = w * h;
    s_i = 0;
    for (s = s_min; s <= s_max; ++s) {
      int idx = o_i + s_i * o_num;
      float* s_ptr = vl_scalespace_get_octave(scsp, o, s);
      mxArray * plane = mxCreateNumericMatrix(w, h, mxSINGLE_CLASS,
                                              mxREAL);
      memcpy(mxGetData(plane), s_ptr, sizeof(float) * num_el);
      mxSetCell(data, idx, plane);
      s_i++;
    }
    o_i ++;
  }
  mxSetFieldByNumber(array, 0, 2, data);
  return array;
}
