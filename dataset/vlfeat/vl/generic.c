...
@par Copyright &copy; 2012 The VLFeat Team
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  -->

<em>The VLFeat C library implements popular computer vision
algorithms, with a special focus on feature detectors and
descriptors. Applications range from wide-baseline matching to image
classification and object detection. VLFeat aims to be simple to use,
portable, and thoroughly documented.</em>
...
- @ref intro
- <b>Computer vision algorithms</b>
  - @ref framedet
  - @ref scalespace
  - @ref dsift
  - @ref mser
...
The VLFeat project is a collaborative effort started in 2007 by Andrea
Vedaldi and Brian Fulkerson at the University of California at Los
Angeles and now developed by the VLFeat Team. The project includes the
development of a C library, implementing a number of popular computer
vision algorithms, as well as wrappers for scripting languages such as
MATLAB.

The reference documentation of the algorithms implemented in VLFeat is
contained in this manual, describing the C API. Other documentation
(for example the help built into VLFeat MATLAB functions) does not
contain a similarly complete and detailed description.

Using the library from a C program is for the most part obvious from
the examples included in the descriptions of the various algorithms
(see the @ref contents for a list).  The general design of the library
(including for example the handling of errors and the ownership
conventions for allocated objects) is descried in @ref design. Further
information for the library developers is described in the @ref dev.

<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  -->
@page design Design
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  -->

The VLFeat C library is designed to be portable and simple to
integrate with high level languages and environments such as
MATLAB. This section illustrates several general aspects of the
library, namely:

- @ref design-errors
- @ref design-portability
- @ref design-matlab
- @ref design-resources
- @ref design-objects
- @ref design-threads

<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  -->
@section design-errors Errors
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  -->

The VLFeat C library handles errors in a manner similar to the standard
C library. On encountering an error a function may either

- return an error code (::VlErrorType) and
If a problemWhen a function encounters an error it maeither returns
an error code directly (::VlError
...
whose name contains either the keywords @c new or @c copy, transfer
the ownership of the memory block or object to the caller. The caller
must dispose explicitly of all the resources he owns (by calling
::vl_free for a memory block, or the appropriate destructor for an
object).
...
Object names are captilaized and start with the <code>Vl</code>
...
invisible changes in the code that are detected by control version
systems such as Git. Therefore whitespaces at the end of lines must be
avoided.</li>

<li><b>Object names.</b> Object names are capitalized and start with
the <code>Vl</code> suffix (e.g. <code>VlSift</code>).</li>

<li><b>Descriptive variable names.</b> Most variable names should
start with a lower case letter and be capitalized, e.g., @c
numElements. Only the following abbreviations are considered
acceptable when constructing variable names: @c num, @c max, @c
min, @c src, @c dst. The @c dimension of a vector is the number of elements it
contains (for other objects that could be a @c size, a @c length, or a
@c numElements). For multi-dimensional arrays, @c dimensions could
indicate the array with each of the @c numDimensions dimensions.

<table>
<caption>Example variable names</caption>
<tr>
 <th>Example</th>
 <th>Comment</th>
</tr>
<tr>
 <td><code>maxNumElements</code></td>
 <td>The maxinum number of elements.</td>
</tr>
<tr>
 <td><code>size</code></td>
 <td>The size of an object, for example the number of bytes in a memory
 block.</td>
</tr>
<tr>
 <td><code>dimension</code></td>
 <td>The dimension of a vector or array. Generally, this is the number
 of elements spanned by one scalar index</td>
</tr>
<tr>
  <td><code>dimensions</code></td>
  <td>An array of dimensions for a multi-dimensional array or tensor.</td>
<tr>
 <td><code>numDimensions</code></td>
 <td>The number of dimensions of a multi-dimensional array or tensor.</td>
</tr>
<tr>
 <td><code>srcWidth</code></td>
 <td>The width of a source image.</td>
</tr>
</table>
...
fine to use short index names such as @c i, @c j, and @c k. For
example:
...
is considered acceptable.</li> <!-- short variable names -->

<li><b>Function and methods names.</b> Function names are lowercase
with words separated by underscores and start with the
<code>vl_</code> prefix. Methods names in particular start with the
object name. Static function names local to a module start with a
<code>_vl_</code> prefix instead.

<table>
<caption>Example function and method names</caption>
<tr>
 <th>Example</th>
 <th>Comment</th>
</tr>
<tr>
 <td><code>vl_get_version_string</code></td>
 <td>A function name.</td>
</tr>
<tr>
<td><code>vl_sift_get_num_frames</code></td>
<td>A method name for the object <code>VlSift</code>.</td>
</tr>
<tr>
 <td><code>_vl_smooth</code></td>
 <td>A name of a static function local to a module.</td>
</tr>
</table>

</li> <!-- function and method names -->

<li><b>Code alignment.</b> Indendtation uses two spaces (and no tabs).
Explicitly aligning blocks of code across multiple lines is
not encouraged.</li>

<li><b>Function declarations.</b> Functions can be declared on
one or multiple lines. In the latter case, the output arguments
are contained in one line. Modifiers such as ::VL_EXPORT and ::VL_INLINE
are contained in the output argument line. For example:

@code
VL_EXPORT vl_bool vl_some_function () ;
...
<li><b>Function definition.</b> Function definitions are similar
to function declaration. However, they do not contain the ::VL_EXPORT
modifier and are always in at least two lines. For example:
...
Thi section describes how to document the VLFeat C library.  VLFeat
code contains its own in documentation <a
...
VLFeat documentation makes use of
a bibliographic reference database in BibTeX format (see the file @c
docsrc/vlfeat.bib). Doxygen uses this file to interpret instances of
the <code>@@cite{xyz}</code> command, where @c xyz is a BibTeX
...
Then the Doxygen directive
<code>@@cite{martin97the-det-curve}</code> generates the output
@cite{martin97the-det-curve}, which includes a link to the corresponding
...
VLFeat provides a few C preprocessor macros of general utility. These
include stringification (::VL_STRINGIFY, ::VL_XSTRINGIFY) and
concatenation (::VL_CAT, ::VL_XCAT) of symbols.
