...
/** ------------------------------------------------------------------
 ** @name Unrecoverable error handling
 ** @{ */

/** @def VL_ASSERT
 ** @brief Print message and assert if condition not satisfied.
 ** @note TODO solve if to change this in case of mex files - instead of assert
 **       mexErrMsgIdAndTxt shoudl be called.
 **/
#define VL_ASSERT(condition,...)                                            \
  do {                                                                      \
    if(!(condition)) {                                                      \
      VL_PRINTF(__VA_ARGS__);                                               \
      assert(condition);                                                    \
    }                                                                       \
  } while(0)                                                                \
